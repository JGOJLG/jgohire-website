...
function formatCurrency(amount: number | null | undefined, currency: string | null | undefined) {
  return new Intl.NumberFormat("en-US", { style: "currency", currency: (currency || "usd").toUpperCase() }).format(Number(amount ?? 0) / 100);
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (supabase && customerEmail && (session.payment_status === "paid" || session.payment_status === "no_payment_required")) {
      const { error: grantError } = await supabase.from("member_entitlement_grants").upsert({
        email: customerEmail,
        entitlement: "linkedin_guide",
        source: "stripe_checkout",
        external_reference: session.id,
      }, { onConflict: "email,entitlement,external_reference", ignoreDuplicates: true });
    ...
    return NextResponse.json({ received: true, emailSent: true, entitlementRecorded, emailId: emailResult.data?.id ?? null });
  } catch (error) {
    const detail = getErrorMessage(error);
    console.error("Unable to process Stripe purchase webhook:", detail);
